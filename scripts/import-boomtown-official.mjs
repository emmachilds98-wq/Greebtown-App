...
//
// Regenerates the `artists` array in js/app.js between the
// AUTO-GENERATED:LINEUP markers, same as the Clashfinder sync script, so
// both sources feed the same downstream data model
// (reconcileSavedArtists(), customArtists, buildSyncPayload() etc.).
...
// scripts/sync-boomtown-lineup.mjs.
const FESTIVAL_DAY_CUTOFF_HOUR = 10;

// The official app's export arrives as UTF-8 text that's been mis-decoded
// as windows-1252 somewhere in the capture pipeline (accented names like
// "Adélia" show up as "AdÃ©lia"). Reverse it where it's detectable.
const CP1252_HIGH = {
...
function buildArtistsBlock(rawRows) {
  const rows = rawRows
    .map((r) => ({
...
async function main() {
  ...
  console.log(`Built ${count} valid line-up entries.`);
