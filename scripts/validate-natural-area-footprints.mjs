...
const errors = [], ids = new Set(), sourceNames = new Set();
...
for (const [index, area] of (data.footprints || []).entries()) {
  ...
  ids.add(area.id);
  if (!area.sourceName || !runtime.includes(`name:"${area.sourceName}"`) || sourceNames.has(area.sourceName)) errors.push(`${prefix}: sourceName must uniquely match a runtime named place`);
  sourceNames.add(area.sourceName);
  ...
  if (typeof area.notes !== "string" || area.notes.length < 24) errors.push(`${prefix}: requires a concise visual-review note`);
