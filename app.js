...
// Multi-select genre filter — any number of genre chips can be active at
// once; an artist matches if their genre is in this set (or if the set
// is empty, everyone matches). Combines with the free-text search below
// as an AND, so "Techno chip + typing 'john'" narrows to Techno acts
// named John, rather than one replacing the other.
let activeGenres = new Set();

function syncGenreChipClasses(){
  document.querySelectorAll("#genreChips .chip").forEach(c=>{
    c.classList.toggle("active", activeGenres.has(c.dataset.g));
  });
}

function updateClearArtistSearchBtn(){
  if(!clearArtistSearchBtn) return;
  clearArtistSearchBtn.style.display = (artistSearch.value.trim().length || activeGenres.size) ? "" : "none";
}
...
function currentFilteredArtists(){
  ...
  return allArtists().filter(artist=>{
    ...
    const matchesGenres = activeGenres.size === 0 || activeGenres.has(genreOf(artist));
...
// Single place that decides whether to show the "start typing" prompt or
// the actual (possibly genre-filtered) results — called after any change
// to either the text box or the genre chips.
function renderArtistResultsFromState(){
  if(artistSearch.value.trim().length === 0 && activeGenres.size === 0){ promptArtistSearch(); return; }
  showArtists(currentFilteredArtists());
}
let artistSearchDebounceTimer = null;
artistSearch.oninput = ()=>{
  // Typing now combines with any active genre chips rather than clearing
  // them, so genre chips stay highlighted while you narrow further by name.
  updateClearArtistSearchBtn();
  clearTimeout(artistSearchDebounceTimer);
  artistSearchDebounceTimer = setTimeout(renderArtistResultsFromState, 180);
};
if(clearArtistSearchBtn){
  clearArtistSearchBtn.onclick = ()=>{
    artistSearch.value = "";
    activeGenres.clear();
    syncGenreChipClasses();
    updateClearArtistSearchBtn();
    renderArtistResultsFromState();
  };
}
updateClearArtistSearchBtn();
promptArtistSearch();
...
function loadGenreChips(){
  ...
  box.innerHTML = genres.map(g=>`<span class="chip${activeGenres.has(g) ? " active" : ""}" data-g="${g}">${g}</span>`).join("");
  box.querySelectorAll(".chip").forEach(s=>{
    s.onclick = ()=>{
      const g = s.dataset.g;
      if(activeGenres.has(g)) activeGenres.delete(g); else activeGenres.add(g);
      s.classList.toggle("active", activeGenres.has(g));
      updateClearArtistSearchBtn();
      renderArtistResultsFromState();
    };
  });
